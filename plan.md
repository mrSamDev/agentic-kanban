...
## Considered but rejected: model config in `.kanban/config.yaml`

**Ask**: Allow per-role / per-skill model selection via Go-config layer.

**Status**: Rejected — not worth the complexity.

**Why**:
- Agent files (`.pi/agents/*.md`) already have `model:` in YAML frontmatter
- Changing a model = open one file, edit one line
- A Go config + template system would save zero editing steps and add ~3-4h code
  + maintenance + user confusion ("my agent files got overwritten?")
- If in future a setting genuinely needs a config file (default lease, project
  hooks, etc.), add config then. Model routing alone doesn't justify it.

**What we do instead**: Document in README: "Edit `<agent>.md` frontmatter `model:` field to change the model."